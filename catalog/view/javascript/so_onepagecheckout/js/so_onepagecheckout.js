...
function ajaxLoadingOn(){
	...
	if(count_loading===1){
		...
		jQuery('.so-onepagecheckout #so-checkout-confirm-button, .so-onepagecheckout .checkout-register, .so-onepagecheckout .checkout-payment-form, .so-onepagecheckout .checkout-shipping-form, .so-onepagecheckout .checkout-cart, .so-onepagecheckout .confirm-section, .so-onepagecheckout .checkout-shipping-methods, .so-onepagecheckout .checkout-payment-methods, .so-onepagecheckout .coupon-voucher, .so-onepagecheckout .location_change').addClass('checkout-loading');
	}
}
...
function ajaxLoadingOff(){
	...
	if(count_loading===0){
		...
		jQuery('.so-onepagecheckout #so-checkout-confirm-button, .so-onepagecheckout .checkout-register, .so-onepagecheckout .checkout-payment-form, .so-onepagecheckout .checkout-shipping-form, .so-onepagecheckout .checkout-cart, .so-onepagecheckout .confirm-section, .so-onepagecheckout .checkout-shipping-methods, .so-onepagecheckout .checkout-payment-methods, .so-onepagecheckout .coupon-voucher, .so-onepagecheckout .location_change').removeClass('checkout-loading');
	}
}

function so_ajax_login(){
	$.ajax({
		url:'index.php?route=checkout/checkout/login&ajax=true&lang=ua/',
		type:'post',
...
jQuery(document).ready(function($) {
	...
	$(document).on('so_checkout_payment_changed', function(e, value){
		$.ajax({url:'index.php?route=checkout/checkout/save&ajax=true',
			type:'post',
	...
	$(document).on('so_checkout_shipping_changed', function(e, value){
		$.ajax({
			url:'index.php?route=checkout/checkout/save&ajax=true&lang=ua/',
			type:'post',
			data:{shipping_method:value},
			dataType:'json',
			success:function(){
				$.ajax({
					url:'index.php?route=checkout/checkout/cart_update&ajax=true&lang=ua/',
					type:'post',
					dataType:'json',
					success:function(json){
						setTimeout(function(){
	...
	$(document).on('so_checkout_reload_cart',function(e,first){
		$.ajax({
			url:'index.php?route=checkout/checkout/cart&ajax=true&lang=ua/',
			type:'get',
			dataType:'html',
			beforeSend:function(){
				if(!first){
	...
	$(document).on('so_checkout_reload_payment', function(){
		console.log('index.php?route=checkout/checkout/payment&ajax=true&lang=ua/');
		$.ajax({
			url:'index.php?route=checkout/checkout/payment&ajax=true&lang=ua/',
			type:'get',
			dataType:'html',
			beforeSend:function(){
				ajaxLoadingOn();
	...
	$(document).on('so_checkout_coupon_voucher_reward_changed',function(e, value){
		$.ajax({
			url:'index.php?route=checkout/checkout/change_coupon_voucher_reward&ajax=true&lang=ua/',
			type:'post',
	...
	$('.so-onepagecheckout form.form-shipping select[name="shipping_country_id"]').on('change', function(e, first){
		...
		$.ajax({
			url:'index.php?route=checkout/checkout/country&lang=ua/&country_id='+this.value,
			dataType:'json',
			beforeSend:function(){
	...
	$('.so-onepagecheckout #button-confirm').on('click', function($){
		$.ajax({
			type:'get',
			url:'index.php?route=extension/payment/cod/confirm&lang=ua/',
			cache:false,
			beforeSend:function(){
	...
	$(document).delegate('.so-onepagecheckout .confirm-button', 'click', function(){
		...
		$.ajax({
			url:'index.php?route=checkout/checkout/confirm&ajax=true&lang=ua/',
			type:'post',
			data:data,
			dataType:'json',
			beforeSend:function(){
	...
	$(document).on('so_checkout_customer_group_changed', function(e, value){
		$.ajax({
			url:'index.php?route=checkout/checkout&ajax=true&lang=ua/',
			type:'get',
	...
	$(document).on('so_checkout_reload_shipping', function(){
		$.ajax({
			url:'index.php?route=checkout/checkout/shipping&ajax=true&lang=ua/',
			type:'get',
			dataType:'html',
			beforeSend:function(){
				ajaxLoadingOn();
	...
	$(document).delegate('.so-onepagecheckout .checkout-product .location_change', 'change', function(){
		var key=$(this).attr('data-product-key');
		var qty=$('.so-onepagecheckout input[name="quantity['+key+']"]').val();
		var loc=$('.so-onepagecheckout select[name="location_id['+key+']"]').val();
		$.ajax({
			url:'index.php?route=checkout/checkout/cart_update&ajax=true&lang=ua/',
			type:'post',
			data:{key:key,quantity:qty,location_id:loc},
			dataType:'json',
			beforeSend:function(){
				ajaxLoadingOn();
				$('#cart > button > a > span').button('loading');
				$('.so-onepagecheckout .checkout-cart').addClass('checkout-loading');
				$('.so-onepagecheckout .location_change').addClass('checkout-loading');
			},
			complete:function(){
				ajaxLoadingOff();
	...
	$(document).delegate(' .btn-delete', 'click', function(){
		var key=$(this).attr('data-product-key');
		$.ajax({
			url:'index.php?route=checkout/checkout/cart_delete&ajax=true&lang=ua/',
			type:'post',
			data:{key:key},
			dataType:'json',
			beforeSend:function(){
	...
	$(document).delegate('#button-voucher', 'click', function(){
		$.ajax({
			url:'index.php?route=extension/total/voucher/voucher&lang=ua/',
			type:'post',
	...
	$(document).delegate('#button-coupon', 'click', function(){
		$.ajax({
			url:'index.php?route=extension/total/coupon/coupon&lang=ua/',
			type:'post',
	...
	$(document).delegate('.so-onepagecheckout #button-reward', 'click', function(){
		$.ajax({
			url:'index.php?route=extension/total/reward/reward&lang=ua/',
