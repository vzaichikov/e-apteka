...
<script type="text/javascript">
    ...
    function subscribe_newsletter_popup()
    {
        ...
        if(email != "")
        {
            ...
            {
                $('.show-error').remove();
                $('.send-mail').after('<span class="show-error" style="display:block; color: red;margin-left: 10px"> Invalid Email </span>')
                return false;
            }
            else
            {
                $.ajax({
                    ...
                    success: function(json) {
                        ...
                        if(json.message == "Subscription Successfull") {
                            checkCookie();
                            $('.send-mail').after('<span class="show-error" style="display:block; color: #003bb3;margin-left: 10px"> ' + json.message + '</span>');
                            setTimeout(function () {
                                var this_close = $('.popup-close');
                                this_close.parent().css('display', 'none');
                                this_close.parents().find('#container-module-newletter').removeClass();
                            }, 3000);
