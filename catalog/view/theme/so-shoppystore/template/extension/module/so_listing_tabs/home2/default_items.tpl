	...
	if (!empty($child_items)) {
		...
		foreach ($child_items as $product) {
			...
			<?php if  ($type_show == 'slider' && ($i % $nb_rows == 1 || $nb_rows == 1)) { ?>
				<div class="ltabs-item product-style2">
			<?php } 
			if ($type_show == 'loadmore'){ ?>
				<div class="ltabs-item new-ltabs-item product-style2" >
			<?php } ?>
					...
					<div class="product-item-container">
						<div class="left-block">
							<?php if($product_image){ ?>
								...
								<ul class="list-inline list-button-dev ">
									...
									<li class="so-quickview"> 
										<a  class="hidden" href="<?php echo $product['href'] ?>" target="<?php echo $item_link_target ?>" title="<?php echo $product['name'] ?>"> <i class="fa fa-heart"></i> </a>
									</li> 
						...
						<?php } if($display_add_to_cart || $display_wishlist || $display_compare){?>
							...
							<?php if($display_add_to_cart){?>
								<button class="addToCart btn-button" data-toggle="tooltip" title="<?php echo $objlang->get('button_cart'); ?>" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>');"><?php echo $objlang->get('button_cart'); ?></button>
							<?php } ?>
