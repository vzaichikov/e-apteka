<div class="simplecheckout-block" id="simplecheckout_customer" <?php echo $hide ? 'data-hide="true"' : '' ?> <?php echo $display_error && $has_error ? 'data-error="true"' : '' ?>>
	<?php if ($display_header || $display_login) { ?>
		<div class="checkout-heading panel-heading"><i class="fa fa-user" aria-hidden="true"></i> <?php echo $text_checkout_customer ?><?php if ($display_login) { ?><span class="checkout-heading-button"><a href="javascript:void(0)" class="do-popup-element" onclick="$('#simple_login_modal').trigger('click');"><?php echo $text_checkout_customer_login ?></a></span><?php } ?></div>
    <?php } ?>
	<div class="simplecheckout-block-content">
        ...
        <?php if ($display_login) { ?>  
            <div class="form-group btn-group-register row" style="margin-top:5px;">
                <h3 style="margin-bottom:5px;"><?php echo $text_filldata_from; ?></h3>
        ...
        <?php if (!$cart_has_drugs) { ?>
            <style>
                .row-customer_is_fourteen{display:none!important;}
            </style>
        <?php } ?>
		
		<?php if (!$cart_has_receipt) { ?>
            <style>
                .row-customer_has_e_receipt{display:none!important;}
            </style>
        <?php } ?>
	...
	<?php if (!$cart_has_receipt) { ?>
		<script>
            $(document).ready(function(){
                $('input#customer_has_e_receipt').val('1');
                $('input#customer_has_e_receipt').prop('checked',true);
            });
        </script>
    <?php } ?>
	...
	<?php if (!$cart_has_drugs) { ?>
		<script>
            $(document).ready(function(){
                $('input#customer_is_fourteen').val('1');
                $('input#customer_is_fourteen').prop('checked',true);
            });
        </script>
    <?php } ?>
