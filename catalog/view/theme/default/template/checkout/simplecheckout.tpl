<?php if (!$ajax && !$popup && !$as_module) { ?>
	...
	<style>
		...
		<?php } ?>
		.row-customer_recipient_exists > label{line-height: 0!important;font-size:0px!important;color:white!important;}		
		.row-customer_is_fourteen > label{line-height: 0!important;font-size:0px!important;color:white!important;}
		.row-customer_has_e_receipt > label{line-height: 0!important;font-size:0px!important;color:white!important;}
	</style>
            ...
            <?php if (!$cart_empty) { ?>
                ...
                <?php if ($steps_count > 1 && $menu_type == '2') { ?>
					...
					<?php if ($display_agreement_checkbox) { ?>
						<div id="agreement_checkbox">
							<?php foreach ($text_agreements as $agreement_id => $text_agreement) { ?>
								<div class="checkbox">
									<input style="display:none;" type="checkbox" name="agreements[]" value="<?php echo $agreement_id ?>" <?php echo in_array($agreement_id, $agreements) ? 'checked="checked"' : '' ?> />
									<label class="el-checkbox-style" style="padding-left:0px;"><span class="agreement-text"><?php echo $text_agreement; ?></span></label>
								</div>
							<?php } ?>
						</div>
					<?php } ?>
					...
					<div class="simplecheckout-button-block buttons" id="buttons">
						<div class="simplecheckout-button-right simplecheckout-confirm-button-left">							
							<?php if ($steps_count > 1) { ?>
								<a class="bbtn bbtn-primary product__btn-cart simple__btn-main" data-onclick="nextStep" id="simplecheckout_button_next"><span><?php echo $button_next; ?></span></a>
							<?php } ?>
							<a class="bbtn bbtn-primary product__btn-cart simple__btn-main" <?php echo $block_order ? 'disabled' : '' ?> data-onclick="createOrder" id="simplecheckout_button_confirm"><span><?php echo $button_order; ?></span></a>
						</div>
						<div class="simplecheckout-button-left">
							<?php if ($display_back_button) { ?>
								<a class="bbtn bbtn-primary product__btn-cart simple__btn-main" data-onclick="backHistory" id="simplecheckout_button_back"><span><?php echo $button_back; ?></span></a>
							<?php } ?>
							<?php if ($steps_count > 1) { ?>
								<a class="bbtn bbtn-primary product__btn-cart simple__btn-main" data-onclick="previousStep" id="simplecheckout_button_prev"><span><?php echo $button_prev; ?></span></a>
							<?php } ?>
						</div>
					</div>  
	...
	<script>
		$(document).ready(function(){
			if (document.getElementById("request-method").content == 'GET'){
				passEcommerceStepsToDataLayer('checkoutPageOpened');
			}
		});
	</script>
