<?php
	class ModelAccountCustomer extends Model {
		public function addCustomer($data) {
			...
			if (!empty($data['country_id']) && !empty($data['address_1'])){
		...
		public function editPasswordByCustomerID($customer_id, $password) {
		...
		public function getCustomerByEmail($email) {
			...
			if (checkIfStringIsEmail($email)){
				...
				OR (TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '" . $this->db->escape(preparePhone($email)) . "')"
				);
		...
		public function getTotalCustomersByEmail($email) {
			...
			if (checkIfStringIsEmail($email)){
				...
				$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' OR (TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '" . $this->db->escape(preparePhone($email)) . "')");
			}
