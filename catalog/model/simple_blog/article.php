<?php
	class ModelSimpleBlogArticle extends Model {
		...
		public function getArticles($data = array()) {
			...
			$sql = "SELECT sba.*, sbad.*, sbau.name AS author_name FROM `" . DB_PREFIX . "simple_blog_article` sba
				LEFT JOIN `" . DB_PREFIX . "simple_blog_article_description` sbad ON(sba.simple_blog_article_id=sbad.simple_blog_article_id)
				LEFT JOIN `" . DB_PREFIX . "simple_blog_article_to_store` sbas ON(sba.simple_blog_article_id=sbas.simple_blog_article_id)
				LEFT JOIN `" . DB_PREFIX . "simple_blog_article_to_category` sb2c ON(sba.simple_blog_article_id=sb2c.simple_blog_article_id)
				LEFT JOIN `" . DB_PREFIX . "simple_blog_author` sbau ON(sba.simple_blog_author_id=sbau.simple_blog_author_id)
				WHERE sba.status=1 AND sbau.status=1 AND
				sbas.store_id='" . (int)$this->config->get('config_store_id') . "' AND
				sbad.language_id='" . (int)$this->config->get('config_language_id') . "'";
			
			if(!empty($data['blog_search'])) {
				$sql .= " AND LCASE(sbad.article_title) LIKE '" . $this->db->escape(utf8_strtolower($data['blog_search'])) . "%'";
			}
			
			if(isset($data['simple_blog_category_id'])) {
				$sql .= " AND sb2c.simple_blog_category_id = '" . (int)$data['simple_blog_category_id'] ."'";
			}
			
			$sql .= " ORDER BY sba.date_added DESC";
