...
class ModelCatalogOchelpSpecial extends Model {
	...
	public function getSpecial($special_id) {
		$query = $this->db->query("SELECT DISTINCT *, s.image as oneimage, s.banner as onebanner, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'special_id=" . (int)$special_id . "') AS keyword FROM " . DB_PREFIX . "special s LEFT JOIN " . DB_PREFIX . "special_description sd ON (s.special_id = sd.special_id) WHERE s.special_id = '" . (int)$special_id . "' AND sd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getActiveSpecialByProduct($product_id){
			...
			$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "special s 
			LEFT JOIN " . DB_PREFIX . "special_product sp on (s.special_id = sp.special_id)
	...
	public function getSpecialShort($limit) {
		$query = $this->db->query("SELECT *, s.image as oneimage, s.banner as onebanner FROM " . DB_PREFIX . "special s LEFT JOIN " . DB_PREFIX . "special_description sd ON (s.special_id = sd.special_id) LEFT JOIN " . DB_PREFIX . "special_to_store s2s ON (s.special_id = s2s.special_id) WHERE sd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND s2s.store_id = '" . (int) $this->config->get('config_store_id') . "' AND s.status = '1' ORDER BY s.date_added DESC LIMIT " . (int) $limit);

		return $query->rows;
	}

	public function getTotalSpecial() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "special s LEFT JOIN " . DB_PREFIX . "special_to_store s2s ON (s.special_id = s2s.special_id) WHERE s2s.store_id = '" . (int) $this->config->get('config_store_id') . "' AND s.status = '1' AND date_end > DATE(NOW())");
	...
	public function getSpecialByProductId($product_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "special s LEFT JOIN " . DB_PREFIX . "special_product sp on (s.special_id = sp.special_id) WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getSpecialProduct($product_id, $special_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_product WHERE product_id = '" . (int)$product_id . "' AND special_id = '" . (int)$special_id . "'");
	...
	public function getSpecialProductIdTotal($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_product WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getSpecialProducts($data = array()) {
		$sql = "SELECT DISTINCT sp.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = sp.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "special_product sp LEFT JOIN " . DB_PREFIX . "product p ON (sp.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND sp.special_id = '" . (int)$data['special_id'] . "' GROUP BY sp.product_id";
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT sp.product_id) AS total FROM " . DB_PREFIX . "special_product sp LEFT JOIN " . DB_PREFIX . "product p ON (sp.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND sp.special_id = '" . (int)$special_id . "'");
