<?php
class ModelCheckoutBocorder extends Model {
	public function addBocorder($data) {
	...
	public function getBocorder($order_id) {
	...
	public function addBocorderHistory($order_id, $order_status_id, $exan, $comment = '', $notify = false, $override = false) {
		$order_info = $this->getBocorder($order_id);
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				// Buyer Alert Mail
				if ($order_info['email'] != $this->config->get('config_email') && $order_info['email'] != '') {
					$mail = new Mail();
				...
				if (in_array('order', (array)$this->config->get('config_mail_alert'))) {
					...
					$new_subject = $subject . ' (' . $data['payment_method'] . ')';
					...
					$mail->setFrom($this->config->get('config_email'));
					$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
					$mail->setSubject(html_entity_decode($new_subject, ENT_QUOTES, 'UTF-8'));
					$mail->setHtml($html);
					$mail->setText($text);
					$mail->send();
