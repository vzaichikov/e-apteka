...
class ModelExtensionModuleSosearchpro extends Model {
    ...
    public function getProducts($data = array()) {
		...
		(SELECT SUM(op.quantity) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id)	WHERE o.order_status_id > '0' AND op.product_id = p.product_id GROUP BY op.product_id ORDER BY total) AS sales";
...
//		$sql .= " ORDER BY p.is_onstock DESC, (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE p.price END)";
        $sql .= " ORDER BY p.sale_quantity DESC";
