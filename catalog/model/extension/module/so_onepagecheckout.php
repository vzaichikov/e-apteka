	...
	class ModelExtensionModuleSoOnepageCheckout extends Model {
		...
		public function save($new_data = null) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				'has_pricegroup_discount' => $product['has_pricegroup_discount'],
		...
		public function getTotals() {
			...
			foreach ($totals as $total) {
				...
				if (isset($total['subcode']) AND $total['subcode'] == 'flat.flat'){
					$result[] = array(
					'title' => $total['title'],
					'text'  => $total['value'],
					);
					...
					$result[] = array( 
					'title' => $total['title'],
					'text'  => ($total['value'] == 0)?$this->language->get('free_price'):$this->registry->get('currency')->format($total['value'], $this->session->data['currency'])
					);
				} else {
					$result[] = array(
					'title' => $total['title'],
		...
		public function getProducts() {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				foreach ($product['available_locations'] as $location){
					$available_locations[] = array(
					'location_id' => $location['location_id'],
					'name' => $location['name'],
					'dname' =>  $location['dname'],
					'price' => $this->registry->get('currency')->format($this->tax->calculate($location['price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']),				
					);
				}
                ...
                'name'       => $product['name'],
				'stock'     => $product['stock'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
				'general_price'       => $this->registry->get('currency')->format($this->tax->calculate($product['general_price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']),
				'has_pricegroup_discount' => $product['has_pricegroup_discount'],
                'thumb'      => $image,
                'model'      => $product['model'],
                'option'     => $option_data,
