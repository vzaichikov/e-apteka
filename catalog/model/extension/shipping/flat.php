	...
	class ModelExtensionShippingFlat extends Model {
		function getQuote($address) {
			...
			if ($status) {
				...
				'text'         => $this->language->get('zero_ship_price'),//$this->currency->format($this->tax->calculate($this->config->get('flat_cost'), $this->config->get('flat_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
				);
