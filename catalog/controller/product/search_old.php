    ...
    class ControllerProductSearch extends Controller {
        public function index() {
            ...
            */
            ...
            if (isset($this->request->get['search'])) {
                $search = keyErrFilterENToRU($this->request->get['search']);
            ...
            $this->load->model('catalog/category');
            $this->load->model('hobotix/hobosearch');
            ...
            if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
				...
				$results = $this->model_hobotix_hobosearch->getCategories($filter_data);				
				$exactCategoryMatchID = false;
				$jsonCategories = array();
				foreach ($results as $result){
					$category_info = $this->model_catalog_category->getCategory($result['category_id']);
					
					if (!in_array($result['source'], array('ngram', 'soundex'))){
						$exactCategoryMatchID = $category_info['category_id'];					
					}
                    
                    if ($exactCategoryMatchID){
						break;
					}
                }
                
                if ($exactCategoryMatchID){
					//$this->response->redirect($this->url->link('product/category', 'path=' . $category_info['category_id'] . '&search=' . $search));
                }
                ...
                'value' => 'pd.name-DESC',
                'href'  => $this->url->link('product/search', 'sort=p.order_count&order=DESC' . $url)
                );                            
