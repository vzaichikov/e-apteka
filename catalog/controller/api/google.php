...
class ControllerAPIGoogle extends Controller {	
	...
	private function parseJWTPayloadAndLoginOrRegister($JWTPayload){
        ...
        $customer_query = $this->db->ncquery("SELECT * FROM oc_customer WHERE social_id = '" . (string)$this->db->escape($customer['social_id']) . "'");                
        ...
        if ($customer_info) {
        	...
        	if($customer['email']){ 
        		if ($this->customer->login($customer['email'], '', true)) {                            
        			...
        			$this->db->ncquery("UPDATE oc_customer SET social_id = '" . (string)$this->db->escape($customer['social_id']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
