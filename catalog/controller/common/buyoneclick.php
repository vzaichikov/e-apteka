<?php
	class ControllerCommonBuyoneclick extends Controller {	
		public function info() {
			...
			if ($this->request->post['order-type'] == 'full'){
				$this->request->post['quantity'] = $this->request->post['quantity-main'];
			}
			
			//Сорян, чисто для понимания логики
			if ($this->request->post['order-type'] == 'part'){
				$this->request->post['quantity'] = $this->request->post['quantity'];
			}
			...
			if ($product) {
				...
				$product['total_in_stock'] = $product['quantity'];				
				...
				if (!$json) {
					...
					$data['text_product_is_receipt'] = $this->language->get('text_product_is_receipt');
					
					$data['product']['is_thermolabel'] = $product['is_thermolabel'];
					$data['text_product_is_thermolabel'] = $this->language->get('text_product_is_thermolabel');
					
					$data['product']['no_shipping'] = $product['no_shipping'];
					$data['text_product_no_shipping'] = $this->language->get('text_product_no_shipping');
					
					$data['product']['no_payment'] = $product['no_payment'];
					$data['text_product_no_payment'] = $this->language->get('text_product_no_payment');

					$data['text_reserve1'] = $this->language->get('text_reserve1');
					$data['text_reserve2'] = $this->language->get('text_reserve2');
					...
					$json['success'] = $this->load->view('common/buyoneclick.tpl', $data);
