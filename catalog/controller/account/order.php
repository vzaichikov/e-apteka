<?php
	class ControllerAccountOrder extends Controller {
		public function index() {
			...
			foreach ($results as $result) {
				...
				foreach ($products as $product){
					...
					if ($option_data && $product_info['count_of_parts'] > 1 && $product_info['price_of_part']){
						
						$price = $this->currency->format($this->tax->calculate($product_info['price_of_part'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
				...
				'hide'       		=> $this->url->link('account/order/hide', 'order_id=' . $result['order_id'], true),
				'reorder'			=> $this->url->link('account/order/reorderfull', 'order_id=' . $result['order_id'], true)
				);
			}
		...
		public function hide(){
			...
			$status = (int)$this->model_account_order->setOrderHideNoHide($order_id);
			
			$this->response->setOutput($status);
			
		}
		
		public function reorderfull(){
			...
			if ($order_info) {
				...
				$this->cart->destroyCurrentLocationID();
