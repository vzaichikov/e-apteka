<?php
	class ControllerAccountEdit extends Controller {
		...
		public function index() {
			...
			$data['entry_profileimage'] = $this->language->get('entry_profileimage');
			
			$this->load->model('tool/image');
			if (isset($this->request->post['image'])) {
				$data['image'] = $this->request->post['image'];
				} elseif (!empty($customer_info)) {
				$data['image'] = $this->model_tool_image->resize($customer_info['image'], 100, 100);
				} else {
				$data['image'] = '';
			}
		...
		public function uploadprofileimage() {
			...
			if (!$json) {
				...
				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_IMAGE.'profiletmage/' . $file);
				
				// Hide the uploaded file name so people can not link to it directly.
				$this->load->model('tool/upload');
				
				$json['success'] = $this->language->get('text_upload');
				$json['file'] ='profiletmage/'.$file;
				$file1=$this->model_tool_image->resize('profiletmage/'.$file, 100, 100);
				$json['file1'] = $file1;
			}
