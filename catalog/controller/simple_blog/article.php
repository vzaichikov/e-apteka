	...
	class ControllerSimpleBlogArticle extends Controller {
		public function index() {
			...
			if(!$this->checkDatabase()) {
				...
				setlocale(LC_TIME, $this->language->get('setlocale'));
				...
				foreach($results as $result) {
					...
					if ($result['featured_image']) {
						...
						$image = $this->model_tool_image->resize($result['featured_image'], 500, 325);
						$featured_found = 1;

					} else if($result['image']) {
						$image = HTTP_SERVER . 'image/' . $result['image'];
						$image = $this->model_tool_image->resize($result['image'], 500, 325);
						$featured_found = '';
					} else {
						//$image = false;
						$image = $this->model_tool_image->resize('placeholder.png', 500, 325);
					...
					$data['articles'][] = array(
						...
						'date_added'		=> $this->model_tool_date->declentDate(strftime('%e %B %Y', strtotime($result['date_added']))),
		...
		public function view() {
			...
			if ($simple_blog_article_id) {
				...
				if($article_info) {
					...
					setlocale(LC_TIME, $this->language->get('setlocale'));
					$data['date_added'] = $this->model_tool_date->declentDate(strftime('%e %B %Y', strtotime($article_info['date_added'])));
                    ...
                    foreach($related_articles as $article) {
                        $data['related_articles'][] = array(
    						...
    						'date_added'		=> $this->model_tool_date->declentDate(date('F jS, Y', strtotime($article['date_added']))),
    						'date_modified'		=> $this->model_tool_date->declentDate(date('F jS, Y', strtotime($article['date_modified']))),
		...
		public function comment() {
			...
			foreach ($results as $result) {
				...
				$data['comments'][] = array(
					...
					'date_added' 		=> $this->model_tool_date->declentDate(date($this->language->get('text_date_format_long'), strtotime($result['date_added'])))
				);
			}
