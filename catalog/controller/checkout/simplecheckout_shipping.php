...
class ControllerCheckoutSimpleCheckoutShipping extends SimpleController {
    ...
    public function index() {
        ...
        if ($unavailable_novaposhta && ($unavailable_novaposhta > $this->cart->getSubTotal())){
              $this->_templateData['text_delivery_to_ukraine_unavailable'] = sprintf($this->language->get('text_delivery_to_ukraine_unavailable'), $this->currency->format($this->tax->calculate($unavailable_novaposhta, $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']));
        }
      ...
      //  var_dump($this->_templateData['shipping_methods']);
		...
		$this->_templateData['no_shipping'] = !$this->cart->getIfShippingIsPossible();
		$this->_templateData['text_no_shipping'] = $this->language->get('text_no_shipping');
