		...
		protected function getList() {
			...
			foreach ($results as $result) {
				...
				if (!is_array($columns)) {
					...
					foreach ($columns as $column => $attr) {
						if ($column == 'image') {
							...
							$row['descriptions'] = $this->model_catalog_category->getCategoryDescriptions($result['category_id']);						
				...
				if ((int)$result['status']){
					...
					if (trim($row['descriptions'][2]['name']) == trim($row['descriptions'][3]['name'])){
						$row['problems'][] = 'не переведено имя: UA';
					}		
					
					if (trim($row['descriptions'][2]['name']) == trim($row['descriptions'][4]['name'])){
						$row['problems'][] = 'не переведено имя: EN';
					}	
					
					if (mb_strlen(trim(strip_tags($row['descriptions'][2]['description']))) < 20){
						$row['problems'][] = 'пустое описание';
					}	
					
					if (mb_strlen(trim(strip_tags($row['descriptions'][2]['seo_name']))) < 2){
						$row['problems'][] = 'нет seo-названия';
					}
				}
