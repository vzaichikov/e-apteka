...
class ControllerCatalogReview extends Controller {
	...
	public function addreviews() {
		...
		$r = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product
							  WHERE product_id NOT IN (SELECT distinct product_id FROM " . DB_PREFIX . "review) ");
	
		foreach($r->rows as $product){
			
			$filter_data = array(
				'filter_product_id'    => $product['product_id'],
			);
	
			
			$rresults = $this->model_catalog_review->getReviews($filter_data);
			...
			if($rresults){
				...
				foreach ($rresults as $result) {
					...
					if((int)$result['rating'] < 3){
						
						//echo '<br>+++'.$rating .' '.$product['product_id'];
						$this->db->query("UPDATE " . DB_PREFIX . "review SET rating = '" . (int)$rating . "' WHERE review_id = '" . (int)$result['review_id'] . "'");
