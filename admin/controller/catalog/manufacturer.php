...
//quicksave end
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$problems = array();
			$products = array();
			$total_products = $this->model_catalog_product->getTotalProductsByManufacturerId($result['manufacturer_id']);
			
			if ($total_products <= 3){
				$problems[] = 'товаров меньше трех';
				
				$products = $this->model_catalog_product->getProducts(array('start' => 0, 'limit' => 5, 'filter_manufacturer_id' => $result['manufacturer_id']));
				$quantity = 0;
				...
				if (!$quantity){
					$problems[] = 'нет наличия';
				}
			}
			
			$descriptions = $this->model_catalog_manufacturer->getManufacturerDescriptions($result['manufacturer_id']);
			
			if ((trim($descriptions[2]['name']) == trim($descriptions[3]['name']) || trim($descriptions[3]['name']) == trim($descriptions[4]['name']))){
				$problems[] = 'идентичные названия ru ua';
			}
			
			if (!trim($descriptions[2]['country'])){
				$problems[] = 'не опознается страна';
			}
...
//quicksave end
		...
		if (isset($this->request->post['uuid'])) {
			...
			$data['uuid'] = $manufacturer_info['uuid'];
		} else {
			$data['uuid'] = '';
		}
