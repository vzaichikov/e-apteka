...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		$starts = 0;
		if(isset($_GET['start'])) $starts = (int)$_GET['start'];
/*		
		$ban_ips = array();
		$r = $this->db->query("SELECT ip FROM " . DB_PREFIX . "ip_register");
		if($r->num_rows){
		...
		$result = $this->db->query('SELECT customer_id, firstname FROM oc_customer WHERE ip IN ("'.implode('","',$ban_ips).'")');
		foreach($result->rows as $row){
			$this->model_customer_customer->deleteCustomer($row['customer_id']);
		}
		
		$result = $this->db->query('SELECT customer_id, ip, firstname FROM oc_customer ORDER BY customer_id DESC LIMIT '.$starts.', 100000');// WHERE firstname = "newsletter"');
		foreach($result->rows as $row){
			
			if(is_numeric($row['firstname'])){
				$this->model_customer_customer->deleteCustomer($row['customer_id']);
			}else{
				//$this->model_customer_customer->deleteCustomer($row['customer_id']);
			}
