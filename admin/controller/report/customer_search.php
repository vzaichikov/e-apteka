...
class ControllerReportCustomerSearch extends Controller {
	public function index() {
		...
		foreach ($results as $result) {			
			...
			$jResults = array();
			$json_results = $result['results']?json_decode($result['results'], true):array();
			
			if (!empty($json_results['c'])){
				foreach ($json_results['c'] as $category_id){
					if ($category_info = $this->model_catalog_category->getCategory($category_id)){					
						$jResults[] = '<i class="fa fa-bars"></i> ' . $category_info['name'];
					}
				}
			}
			
			if (!empty($json_results['p'])){
				foreach ($json_results['p'] as $product_id){
					if ($product_info = $this->model_catalog_product->getProduct($product_id)){					
						$jResults[] = '<i class="fa fa-check"></i> ' . $product_info['name'];
					}
				}
			}
