...
class ControllerExtensionModuleSoconfig extends Controller {
	...
	public function  __construct($registry) {
		...
		$this->typelayouts = array(
			...
			'typefooter'=> array('key'=>'1', 'title'=>'Footer 1 (used in Layout 1,2 & 3)'),
			),
			array(
			'key'=>'2',
			...
			array(
			'key'=>'4',
			'typelayout'=>'<p>Layout 4</p> <p class="font-small">(Header - Type 4,Footer - Type 2)</p>',
			'typeheader'=> array('key'=>'4', 'title'=>'Header 4 (used in Layout 4)'),
			'typefooter'=> array('key'=>'2', 'title'=>'Footer 2 (used in Layout 4)'),
			),
			array(
			'key'=>'5',
			...
			'typefooter'=> array('key'=>'3', 'title'=>'Footer 3 (used in Layout 5)'),
			),
		);
    ...
    public function index() {
			...
			if($this->request->post['buttonForm'] == 'color' ){
				...
				if($data['scsscompile'] != 1){
					...
					unset($this->request->post['buttonForm']);
					 $this->response->redirect($this->url->link('extension/module/soconfig', 'token=' . $this->session->data['token'] . '&store_id=' . $store_id, true));
				}else{
		...
		$data['soconfig_href'] = $this->url->link('extension/module/soconfig', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['action'] = $this->url->link('extension/module/soconfig',  'token=' . $this->session->data['token'].'&store_id='.$store_id, 'SSL');
		...
		if($table_exists){
			foreach($store_data as $storeid => $store){
				if($store['status'] && $store['store_id'] == $store_id){
					$load_config = $this->model_soconfig_setting->getSetting($store);
