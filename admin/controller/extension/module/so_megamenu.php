...
class ControllerExtensionModuleSoMegaMenu extends Controller {
    ...
    public function index() {
       ...
       // $this->document->addStyle('view/javascript/so_megamenu/so_megamenu.css');
		...
		$__data = array('sort_order' => 'parent_id DESC name');
		
		$results = $this->model_catalog_category->getCategories($__data);
		
		$_tmp = array();
		foreach ($results as $result) {
			$_tmp[$result['name']] = $result['category_id'];
		}	
		
		ksort($_tmp);
			
		foreach ($_tmp as $name => $id) {
                ...
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if(array_key_exists('category',$this->request->post['content']['subcategory'])){
							if(isset($this->request->post['content']['subcategory']['category'])) $this->request->post['content']['subcategory']['category'] = serialize ($this->request->post['content']['subcategory']['category']);	
