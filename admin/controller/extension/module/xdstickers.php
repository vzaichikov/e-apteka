	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($this->request->post['xdstickers_apply']) {
				$this->response->redirect($this->url->link('extension/module/xdstickers', 'token=' . $this->session->data['token'], true));
			}
...
.xdstickers {padding:0 10px; margin-bottom:5px;}
		...
		$data['action'] = $this->url->link('extension/module/xdstickers', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/xdstickers.tpl', $data));
	}
