	...
	class ControllerSimpleBlogArticle extends Controller {
			...
			if (isset($this->request->get['article_name'])) {
                ...
                if($article_name) {
    				...
    				foreach ($results as $result) {
    					...
    					'primenenie'       		=> strip_tags(html_entity_decode($result['primenenie'], ENT_QUOTES, 'UTF-8')),	
		...
		private function validateForm() {
			...
			foreach ($this->request->post['article_description'] as $language_id => $value) {
				if ((mb_strlen($value['article_title']) < 3) || (mb_strlen($value['article_title']) > 100)) {
