<?php
	class ModelCustomerCustomerGroup extends Model {
		public function addCustomerGroup($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_group SET approval = '" . (int)$data['approval'] . "', uuid = '" . $this->db->escape($data['uuid']) . "', sort_order = '" . (int)$data['sort_order'] . "'");
		...
		public function editCustomerGroup($customer_group_id, $data) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer_group SET approval = '" . (int)$data['approval'] . "', uuid = '" . $this->db->escape($data['uuid']) . "', sort_order = '" . (int)$data['sort_order'] . "' WHERE customer_group_id = '" . (int)$customer_group_id . "'");
			...
			if (isset($data['customer_group_price_group'])) {

				$this->db->query("DELETE FROM " . DB_PREFIX . "price_group_to_customer_group WHERE customer_group_id = '" . (int)$customer_group_id . "'");
				
				foreach ($data['customer_group_price_group'] as $pricegroup_id => $value) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "price_group_to_customer_group SET customer_group_id = '" . (int)$customer_group_id . "', pricegroup_id = '" . (int)$pricegroup_id . "', plus = '" . (int)$value['plus'] . "', percent = '" . (float)$value['percent'] . "'");
				}
			}
		}
		
		public function prepareCustomerGroupData($customer_group_id) {
			$this->load->model('catalog/pricegroup');
			
			$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "price_group_to_customer_group (SELECT pricegroup_id, '" . $customer_group_id . "', 0, 0 FROM " . DB_PREFIX . "price_group)");	
