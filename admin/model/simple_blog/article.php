	...
	class ModelSimpleBlogArticle extends Model {
		...
		public function addArticle($data) {
			...
			foreach ($data['article_description'] as $language_id => $value) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_article_description` SET
								 simple_blog_article_id = '" . (int)$simple_blog_article_id . "',
								 language_id = '" . (int)$language_id . "', article_title = '" .
								 $this->db->escape($value['article_title']) . "',
								 primenenie = '" . $this->db->escape($value['primenenie']) . "',
      ...
      if (isset($data['article_addition_description']))
      {
  			...
  			foreach($data['article_addition_description'] as $key => $additional_value) {
  				foreach($additional_value as $val_key => $value) {
  					$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_article_description_additional` SET
									 simple_blog_article_id='" . (int)$simple_blog_article_id . "',
									 language_id='" . (int)$val_key . "', additional_description='" .
									 $this->db->escape($value['additional']) . "'");
  				}
  			}
      }  
      ...
      if (isset($data['article_addition_description']))
      {
		...
		public function getArticles($data = array()) {
			$sql = "SELECT sba.*, sbad.*, sbau.name AS author_name, sbad.article_title AS article_title FROM `" . DB_PREFIX . "simple_blog_article` sba LEFT JOIN `" . DB_PREFIX . "simple_blog_article_description` sbad ON(sba.simple_blog_article_id=sbad.simple_blog_article_id) LEFT JOIN `" . DB_PREFIX . "simple_blog_author` sbau ON(sba.simple_blog_author_id = sbau.simple_blog_author_id) WHERE sbad.language_id='" . (int)$this->config->get('config_language_id'). "'";
			
			if(isset($data['filter_article']) && !empty($data['filter_article'])) {
				$sql .= " AND (sbad.article_title LIKE '" . $this->db->escape($data['filter_article']) . "%'";
				$sql .= " OR sbad.primenenie LIKE '" . $this->db->escape($data['filter_article']) . "%'";
				$sql .= " OR sbad.tags LIKE '" . $this->db->escape($data['filter_article']) . "%')";
			}
