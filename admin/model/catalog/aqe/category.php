...
class ModelCatalogAqeCategory extends Model {
	...
	public function getCategories($data = array()) {
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS c.*, cd2.name AS short_name, cd2.alternate_name AS alternate_name, cp.category_id AS category_id, GROUP_CONCAT(c2.category_id ORDER BY cp.level SEPARATOR '_') AS category_path, GROUP_CONCAT(cd.name ORDER BY cp.level SEPARATOR ' &gt; ') AS name";
