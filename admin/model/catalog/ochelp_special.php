	...
	class ModelCatalogOchelpSpecial extends Model {
		...
		private function editProductSpecialData($data){
				...
				if (((float)$data['price'] + (float)$data['percent'] > 0)){					
					...
					foreach ($this->customerGroups as $customer_group_id){
						if ((float)$data['price']){
							...
							customer_group_id = '" . $customer_group_id . "',
							priority = '0',
							price = '" . (float)($product['price'] - ($product['price']/100*$data['percent'])) . "',
							date_start = DATE(NOW() - INTERVAL 2 DAY),
							date_end = '" . $this->db->escape(date('Y-m-d', strtotime($data['date_end']))) . "',
							timer = 0,
							product_special_group_id = 0
			...
			public function addSpecial($data) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "special SET date_end = '" . $this->db->escape($data['date_end']) . "', sort_order = '" . (int)$data['sort_order'] . "', counter = '" . (int)$data['counter'] . "', total = '" . (int)$data['total'] . "', show_title = '" . (int)$data['show_title'] . "', status = '" . (int)$data['status'] . "', date_added = now()");
			...
			public function editSpecial($special_id, $data) {
				$this->db->query("UPDATE " . DB_PREFIX . "special SET date_end = '" . $this->db->escape($data['date_end']) . "', sort_order = '" . (int)$data['sort_order'] . "', counter = '" . (int)$data['counter'] . "', total = '" . (int)$data['total'] . "',  show_title = '" . (int)$data['show_title'] . "', status = '" . (int)$data['status'] . "' WHERE special_id = '" . (int)$special_id . "'");
