<?php
	class ModelLocalisationLocation extends Model {
		public function addLocation($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "location SET name = '" . $this->db->escape($data['name']) . "', address = '" . $this->db->escape($data['address']) . "', geocode = '" . $this->db->escape($data['geocode']) . "', gmaps_link = '" . $this->db->escape($data['gmaps_link']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', image = '" . $this->db->escape($data['image']) . "', open = '" . $this->db->escape($data['open']) . "', open_struct = '" . $this->db->escape($data['open_struct']) . "', node_id = '" . (int)$data['node_id'] . "', is_stock = '" . (int)$data['is_stock'] . "', can_sell_drugs = '" . (int)$data['can_sell_drugs'] . "', temprorary_closed = '" . (int)$data['temprorary_closed'] . "', information_id = '" . (int)$data['information_id'] . "', default_price = '" . (int)$data['default_price'] . "', sort_order = '" . (int)$data['sort_order'] . "', delivery_times = '" . $this->db->escape($data['delivery_times']) . "', comment = '" . $this->db->escape($data['comment']) . "', uuid = '" . $this->db->escape($data['uuid']) . "'");
			
			$location_id = $this->db->getLastId();
			
			foreach ($data['location_description'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "location_description SET location_id = '" . (int)$location_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', open = '" . $this->db->escape($value['open']) . "', comment = '" . $this->db->escape($value['comment']) . "', address = '" . $this->db->escape($value['address']) . "', delivery_times = '" . $this->db->escape($value['delivery_times']) . "'");
			}
