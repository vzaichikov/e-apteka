...
class SimpleCheckout extends Simple {
    ...
    public function prepareShippingMethods($quote) {
        ...
        if (empty($checkedMethod) || (!empty($checkedMethod) && empty($checkedMethod['useTitle']) && empty($checkedMethod['methods']))) {
            if (!$displayTitles) {
                unset($result['title']);
            }
    ...
    private function copyNameFields() {
        foreach (array('firstname', 'lastname') as $field) {
            ...
            if ((!$this->hasBlock('payment_address') || !$this->isFieldUsed($field, 'payment_address')) && $this->isFieldUsed($field, 'customer')) {
                $this->session->data['simple']['payment_address'][$field] = $this->session->data['simple']['customer'][$field];
            }

            if ((!$this->hasBlock('shipping_address') || !$this->isFieldUsed($field, 'shipping_address')) && $this->isFieldUsed($field, 'customer')) {
