...
class Session {
	...
	public function __construct($adaptor = 'native', $registry = null) {
	...
	public function start($key = 'PHPSESSID', $value = '') {		
		...
		if ($key != 'PHPSESSID') {
			setcookie($key, $this->session_id, $this->adaptor->expire, ini_get('session.cookie_path'), ini_get('session.cookie_domain'), ini_get('session.cookie_secure'), ini_get('session.cookie_httponly'));
		}
	...
	public function destroy($key = 'PHPSESSID') {
		...
		setcookie($key, '', time() - 2592000, ini_get('session.cookie_path'), ini_get('session.cookie_domain'));
	}
}